property pTempWaitList, pWaitList, pTaskList, pCastLibCount, pSysCastNum, pBinCastNum, pNullCastName, pFileExtension, pLoadedCasts, pLatestTaskID, pCurrentDownLoads, pLastError, pDontProfile, pAvailableDynCasts, pPermanentLevelList
...
on startCastLoad me, tCasts, tPermanentFlag, tAdd, tDoIndexing, tDoTracking 
  ...
  if tDoTracking then
    tCastsStr = "" & tCasts
    tCastsStr = replaceChunks(tCastsStr, "\"", "")
    tCastsStr = replaceChunks(tCastsStr, " ", "")
    tCastsStr = replaceChunks(tCastsStr, "[", "")
    tCastsStr = replaceChunks(tCastsStr, "]", "")
    tCastsStr = replaceChunks(tCastsStr, "\r", "")
  end if
  pTempWaitList = []
  tCastList = []
...
on prepare me 
  if count(pTaskList) > 0 then
    ...
    if pDontProfile then
      ...
      i = 1
      repeat while i <= pCurrentDownLoads.count
        tTask = pCurrentDownLoads.getAt(i)
...
on getDomainAndTld me, tURL 
  ...
  tTldItemCount = 1
  tDomain = ""
  if tURL.count(#item) = 1 then
    tDomain = tURL
  else
    if tURL.count(#item) = 2 then
      tDomain = tURL.getProp(#item, 1)
    else
      tTldCount = 1
      tDomainStrLength = tURL.getPropRef(#item, tURL.count(#item) - 1).length
      if tDomainStrLength = 2 or tDomainStrLength = 3 then
        tTldItemCount = 2
      end if
      tDomain = tURL.getProp(#item, tURL.count(#item) - tTldItemCount)
    end if
  end if
...
on DoneCurrentDownLoad me, tFile, tURL, tID, tstate 
  ...
  if tstate <> #done then
    pLastError = netError(pCurrentDownLoads.getAt(tFile).pNetId)
  end if
