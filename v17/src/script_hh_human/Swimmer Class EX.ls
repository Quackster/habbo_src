...
on setPartLists me, tmodels 
  ...
  me.arrangeParts()
  i = 1
  repeat while i <= me.count(#pPartIndex)
    tPartSymbol = pPartIndex.getPropAt(i)
    tPartObj = me.getProp(#pPartList, me.getProp(#pPartIndex, i))
    pPelleFigure.addProp(tPartSymbol, tmodels.getAt(tPartSymbol))
...
on fixSwimmerFigure me, tFigure 
  ...
  repeat while ["bd", "lh", "rh"] <= undefined
    ...
    if voidp(tFigure.getAt(f)) then
      tFigure.setAt(f, ["model":"001", "color":rgb("#EEEEEE")])
    end if
  end repeat
  tFigure.getAt("bd").setAt("model", "s" & tFigure.getAt("bd").getAt("model").getProp(#char, 2, 3))
  tFigure.getAt("lh").setAt("model", "s" & tFigure.getAt("bd").getAt("model").getProp(#char, 2, 3))
  tFigure.getAt("rh").setAt("model", "s" & tFigure.getAt("bd").getAt("model").getProp(#char, 2, 3))
  return(tFigure)
end
