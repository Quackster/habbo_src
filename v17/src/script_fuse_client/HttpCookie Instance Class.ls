...
on handleContentResponse me, tMsg, tContent
  ...
  if (tContent starts "HTTP/") then
    ...
    tEndOfResult = (tBody.char[(tBody.length - 4)] = (("0" & pCRLF) & pCRLF))
    ...
    if (tContent.char[(tContent.length - 4)] = (("0" & pCRLF) & pCRLF)) then
      tContent = tContent.char[1]
      tFinished = 1
    end if
...
on parseRawBody me, tRawbody
  ...
  repeat while 1
    ...
    tRawbody = tTemp[2]
    put tRawbody.char[1] after tBody
...
on urlEncode me, tStr
  ...
  repeat with i = 1 to tStr.length
    ...
    put ("%" & rgb(charToNum(tChar), 0, 0).hexString().char[2]) after tEncodedStr
  end repeat
  return tEncodedStr
end
