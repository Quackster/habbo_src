property pEntryVisual, pBottomBar, pSignSprList, pSignSprLocV, pItemObjList, pUpdateTasks, pViewMaxTime, pViewOpenTime, pViewCloseTime, pAnimUpdate, pFirstInit, pInActiveIconBlend, pMessengerFlash, pNewMsgCount, pNewBuddyRequests, pClubDaysCount
...
on showHotel me
  if not visualizerExists(pEntryVisual) then
    ...
    i = 1
    repeat while 1
      tSpr = tVisObj.getSprById(("water" & i))
      if (tSpr <> 0) then
        tObj = createObject(#temp, "Entry Water Class")
        tObj.define(tSpr.member)
        pItemObjList.add(tObj)
      else
        exit repeat
      end if
    ...
    tSpr = tVisObj.getSprById("train1")
    if (tSpr <> 0) then
      tObj = createObject(#temp, "Entry Train Class")
      tObj.define(tSpr.member)
      pItemObjList.add(tObj)
    end if
    tObj = createObject(#temp, "Entry Scroller Class")
    pItemObjList.add(tObj)
    tObj = createObject(#temp, "Entry Bubbles Class")
    pItemObjList.add(tObj)
  end if
