property pGeometry, pSprite, pMatteSpr, pShadowSpr, pClass, pName, pExtraObjs, pCanvasSize, pMember, pPeopleSize, pInfoStruct, pCustom, pDirection, pXFactor, pLocX, pLocY, pLocH, pHeadDir, pSync, pDancing, pPartList, pCorrectLocZ, pRestingHeight, pFlipList, pPartIndex, pScreenLoc, pMainAction, pCtrlType, pModState, pTrading, pCarrying, pCurrentAnim, pAnimCounter, pEyesClosed, pSleeping, pTalking, pMoving, pMoveStart, pMoveTime, pDestLScreen, pStartLScreen, pWaving, pChanges, pDefShadowMem, pBuffer, pShadowFix, pAlphaColor, pUpdateRect, pPartClass, pColors, pLastDir
...
on getInfo me 
  ...
  if (pName = tSession.get("user_name")) then
    ...
    tBadge = tSession.get("badge_type")
    if tBadge <> 0 then
      pInfoStruct.setAt(#badge, tBadge)
      pInfoStruct.setAt(#badge_visible, tSession.get("badge_visible"))
    end if
  end if
  if pTrading then
  ...
  if (pName = tSession.get("user_name")) then
    ...
    if not tSession.exists("badge_visible") then
      ...
      if not tSession.get("badge_visible") then
        getObject(#room_component).getRoomConnection().send("HIDEBADGE")
      end if
    end if
  end if
end
