...
on refresh me, tdata
  ...
  repeat with i = 1 to pItemListMe.count
    ...
    tColor = pItemListMe[i][#stripColor]
    if not voidp(pItemListMe[i][#props]) then
      tClass = (tClass & pItemListMe[i][#props])
    end if
    ...
    if voidp(pMySlotProps[tItemType]) then
      tAddToSlot = (pMySlotProps.count + 1)
      if tWndObj.elementExists(("trading_mystuff_" & tAddToSlot)) then
        pMySlotProps.addProp(tItemType, ["count": 1, "slot": tAddToSlot, "name": pItemListMe[i][#name]])
        tWndObj.getElement(("trading_mycount_" & tAddToSlot)).setText(pMySlotProps[tItemType]["count"])
      ...
      tCount = pMySlotProps[tItemType]["count"]
      tSlot = pMySlotProps[tItemType]["slot"]
      pMySlotProps[tItemType]["count"] = (tCount + 1)
      tWndObj.getElement(("trading_mycount_" & tSlot)).setText(pMySlotProps[tItemType]["count"])
    end if
  ...
  repeat with i = 1 to pItemListHe.count
    ...
    tColor = pItemListHe[i][#stripColor]
    ...
    if voidp(pHerSlotProps[tItemType]) then
      tAddToSlot = (pHerSlotProps.count + 1)
      if tWndObj.elementExists(("trading_herstuff_" & tAddToSlot)) then
        pHerSlotProps.addProp(tItemType, ["count": 1, "slot": tAddToSlot, "name": pItemListHe[i][#name]])
        tWndObj.getElement(("trading_hercount_" & tAddToSlot)).setText(pHerSlotProps[tItemType]["count"])
    ...
    tCount = pHerSlotProps[tItemType]["count"]
    tSlot = pHerSlotProps[tItemType]["slot"]
    pHerSlotProps[tItemType]["count"] = (tCount + 1)
    tWndObj.getElement(("trading_hercount_" & tSlot)).setText(pHerSlotProps[tItemType]["count"])
  end repeat
