...
on refresh me, tdata 
  ...
  repeat while i <= pItemListMe.count
    ...
    tColor = pItemListMe.getAt(i).getAt(#stripColor)
    if not voidp(pItemListMe.getAt(i).getAt(#props)) then
      tClass = tClass & pItemListMe.getAt(i).getAt(#props)
    end if
    if ilk(tColor, #color) then
      tItemType = tClass & tColor.hexString()
    else
      tItemType = tClass
    end if
    if voidp(pMySlotProps.getAt(tItemType)) then
      tAddToSlot = (pMySlotProps.count + 1)
      if tWndObj.elementExists("trading_mystuff_" & tAddToSlot) then
        pMySlotProps.addProp(tItemType, ["count":1, "slot":tAddToSlot, "name":pItemListMe.getAt(i).getAt(#name)])
        tWndObj.getElement("trading_mycount_" & tAddToSlot).setText(pMySlotProps.getAt(tItemType).getAt("count"))
      ...
      tCount = pMySlotProps.getAt(tItemType).getAt("count")
      tSlot = pMySlotProps.getAt(tItemType).getAt("slot")
      pMySlotProps.getAt(tItemType).setAt("count", (tCount + 1))
      tWndObj.getElement("trading_mycount_" & tSlot).setText(pMySlotProps.getAt(tItemType).getAt("count"))
    end if
  ...
  repeat while i <= pItemListHe.count
    ...
    tColor = pItemListHe.getAt(i).getAt(#stripColor)
    ...
    if voidp(pHerSlotProps.getAt(tItemType)) then
      tAddToSlot = (pHerSlotProps.count + 1)
      if tWndObj.elementExists("trading_herstuff_" & tAddToSlot) then
        pHerSlotProps.addProp(tItemType, ["count":1, "slot":tAddToSlot, "name":pItemListHe.getAt(i).getAt(#name)])
        tWndObj.getElement("trading_hercount_" & tAddToSlot).setText(pHerSlotProps.getAt(tItemType).getAt("count"))
      ...
      tCount = pHerSlotProps.getAt(tItemType).getAt("count")
      tSlot = pHerSlotProps.getAt(tItemType).getAt("slot")
      pHerSlotProps.getAt(tItemType).setAt("count", (tCount + 1))
      tWndObj.getElement("trading_hercount_" & tSlot).setText(pHerSlotProps.getAt(tItemType).getAt("count"))
    end if
    i = (1 + i)
  end repeat
    ...
    if (tEvent = "trading_confirm_check") then
      if pAcceptFlagMe then
        ...
        if (pHerSlotProps.count = 0) then
          ...
          tWinObj.merge("habbo_tradingalert_dialog.window")
          tWinObj.center()
