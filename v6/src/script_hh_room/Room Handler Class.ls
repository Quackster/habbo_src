      ...
      "S":
        ...
        if (tObj[#colors].char[1] = "*") then
          if (tObj[#colors].item.count > 1) then
            tObj[#stripColor] = rgb(tObj[#colors].item[tObj[#colors].item.count].char[2])
          else
            tObj[#stripColor] = rgb(tObj[#colors].char[2])
          end if
        else
          tObj[#stripColor] = 0
        end if
...
on handle_doorflat me, tMsg
  ...
  tDelim = the itemDelimiter
  the itemDelimiter = TAB
  tLine = tMsg.content.line[2]
  if (tLine = EMPTY) then
    return error(me, "Retarded doorflat data!", #handle_doorflat)
  end if
  tFlat = [:]
  tFlat[#teleport] = tMsg.content.line[1].word[1]
