property pActivePostItId, pText, pcolor, pWindowID, pLocX, pLocY, pIsController, pChanged
...
on close me
  if (pActivePostItId > 0) then
    ...
    tdata = (tColorHex.char[2] && tWindow.getElement("stickies_text_field").getText())
    if (pChanged = 1) then
