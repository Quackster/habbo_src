property pActive, pPause, pClientID, pStripID, pMoveProc, pSprList, pLocList, pLoczList, pGeometry, pLastLoc, pSmallSpr, pSavedDim, pSavedDir, pClientObj, pItemLocStr, pOrigCoord
...
on define me, tClientID, tStripID, tObjType
  ...
  if (tOrigSprList.count > 0) then
    ...
    tSmallName = tInfo[#smallmember]
    if not memberExists(tSmallName) then
      tSmallName = (tOrigSprList[1].member.name.char[1] & "small")
    end if
    if not memberExists(tSmallName) then
...
on moveItem me
  ...
  if (((tClass = "poster") or (tClass contains "post.it")) or (tClass = "photo")) then
    ...
    pSprList[1].castNum = getmemnum((tProps[#direction] && tName.word[2]))
