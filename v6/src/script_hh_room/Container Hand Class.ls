...
on showContainerItems me 
  ...
  repeat while i <= 9
    ...
    if i <= tCount then
      ...
      if tItem.getAt(#member) contains "*" then
        tItem.setAt(#stripColor, void())
      end if
...
on eventProcContainer me, tEvent, tSprID, tParam 
  ...
  if getThread(#room).getInterface().getProperty(#clickAction) <> "moveHuman" then
    if (getThread(#room).getInterface().getProperty(#clickAction) = "tradeItem") then
