  ...
  repeat with f = tFirst to tLast
    if not voidp(pCurrentPageData["productList"][f]["smallPrewImg"]) then
      ...
      if (tmember <> 0) then
        if tWndObj.elementExists(tid) then
          tElem = tWndObj.getElement(tid)
          if not voidp(pCurrentPageData["productList"][f]["smallColorFlag"]) then
            tColorFlag = pCurrentPageData["productList"][f]["smallColorFlag"]
          else
            tColorFlag = 0
          end if
          if (tColorFlag and not voidp(pCurrentPageData["productList"][f]["partColors"])) then
            ...
            if (tColor.char[1] = "*") then
              tColor = rgb(("#" & string(tColor).char[2]))
              tInk = 41
            else
