property pEntryVisual, pSignSprList, pItemObjList, pBottomBar, pAnimUpdate, pAnimCounter, pUpdateTasks, pFirstInit, pNewMsgCount, pNewBuddyRequests, pViewMaxTime, pViewOpenTime, pViewCloseTime, pMessengerFlash, pInActiveIconBlend
...
on prepare me 
  ...
  if pAnimUpdate then
    ...
    if (pAnimCounter = 2) then
      repeat while ["flag1", "flag2"] <= 1
        tid = getAt(1, count(["flag1", "flag2"]))
