property pEntryVisual, pSignSprList, pItemObjList, pBottomBar, pAnimUpdate, pAnimCounter, pUpdateTasks, pFirstInit, pViewMaxTime, pViewOpenTime, pViewCloseTime, pNewMsgCount, pNewBuddyRequests, pMessengerFlash, pInActiveIconBlend
...
on prepare me 
  ...
  if pAnimUpdate then
    ...
    if (pAnimCounter = 2) then
      repeat while ["fountain1", "fountain2", "fountain3", "fountain4", "fountain5"] <= 1
        tid = getAt(1, count(["fountain1", "fountain2", "fountain3", "fountain4", "fountain5"]))
      ...
      tNum2 = integer(tName2.getProp(#char, length(tName2)))
      tNum2 = (tNum2 + 1)
      if tNum2 > 5 then
        tNum2 = random(5)
      end if
