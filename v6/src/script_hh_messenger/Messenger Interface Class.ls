property pWindowTitle, pOpenWindow, pLastOpenWindow, pRoomProps, pBuddyListPntr, pSelectedBuddies, pBuddyListBuffer, pBuddyListBufferWidth, pBuddylistItemHeigth, pBuddyDrawObjList, pLastSearch, pLastGetMsg, pComposeMsg, pSendMode, pEmailSendOK, pBodyPartObjects, pRemoveBuddy, pBuddyDrawNum, pCurrProf, pMsgsStr, pFriendListSwitch, pFriendListObjs, pWriterID_nobuddies, pWriterID_msgedit, pBuddyDrw_writerID_name, pBuddyDrw_writerID_msgs, pBuddyDrw_writerID_last, pBuddyDrw_writerID_text
    ...
    if (tEvent = #mouseUp) then
      case tElemID of
        ...
        "console_friends_help_backbutton":
          me.ChangeWindowView("console_friends.window")
        "console_safety_info":
          getConnection(getVariable("connection.info.id")).send("MESSENGER_C_CLICK", pLastGetMsg[#id])
          openNetPage(getWindow(pWindowTitle).getElement(tElemID).getaProp(#pLinkTarget))
        "console_official_exit":
          getConnection(getVariable("connection.info.id")).send("MESSENGER_C_READ", pLastGetMsg[#id])
          me.ChangeWindowView("console_myinfo.window")
      end case
    else
