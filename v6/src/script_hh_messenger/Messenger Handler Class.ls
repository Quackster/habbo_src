...
on handle_buddylist me, tMsg 
  ...
  repeat while tMessage.online <= 1
    tName = getAt(1, count(tMessage.online))
    tBuddy = tBuddies.getaProp(tName)
    tMessage.buddies.setaProp(tBuddy.getAt(#id), tBuddy)
    tMessage.render.add(tName)
  end repeat
  repeat while tMessage.offline <= 1
    tName = getAt(1, count(tMessage.offline))
    tBuddy = tBuddies.getaProp(tName)
    tMessage.buddies.setaProp(tBuddy.getAt(#id), tBuddy)
    tMessage.render.add(tName)
  end repeat
