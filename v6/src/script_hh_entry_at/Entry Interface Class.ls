...
on showHotel me
  if not visualizerExists(pEntryVisual) then
    ...
    i = 1
    repeat while 1
      tSpr = tVisObj.getSprById(("twink" & i))
      if (tSpr <> 0) then
        tObj = createObject(#temp, "Entry Twinkle Class")
        tObj.define(tSpr, i)
        pItemObjList.add(tObj)
      else
        exit repeat
    ...
    i = 1
    repeat while 1
      tSpr = tVisObj.getSprById(("Waterfall" & i))
      if (tSpr <> 0) then
        tObj = createObject(#temp, "Entry Waterfall Class")
        tObj.define(tSpr, i)
        pItemObjList.add(tObj)
      else
        exit repeat
...
on updateEntryBar me
  ...
  me.updateMessageCount(pNewMsgCount)
  me.updateBuddyrequestCount(pNewBuddyRequests)
