property pTempPassword, pOpenWindow, pWindowTitle, pmode, pOldFigure, pOldSex, pPartChangeButtons, pBodyPartObjects, pPeopleSize, pBuffer, pFlipList, pNameChecked, pLastNameCheck, pPropsToServer, pErrorMsg, pRegProcess, pRegProcessLocation, pVerifyChangeWndID, pTempPasswords, pLastWindow
  ...
  if (tEvent = #keyDown) then
    case the keyCode of
      ...
      49:
        return 1
      51:
        tPassLength = pTempPasswords[tSprID].length
        if (tPassLength > 0) then
          pTempPasswords[tSprID] = chars(pTempPasswords[tSprID], 1, (tPassLength - 1))
        end if
      117:
        pTempPasswords[tSprID] = EMPTY
      otherwise:
        ...
        if ((not tValidKeys contains the key) and (not tValidKeys = EMPTY)) then
          return 1
        end if
        ...
        if (tPasswordFields.getPos(tSprID) > 0) then
          ...
          pTempPasswords[tSprID] = (pTempPasswords[tSprID] & the key)
          tStr = EMPTY
          repeat with i = 1 to pTempPasswords[tSprID].length
            tStr = (tStr & "*")
          end repeat
          getWindow(tWndID).getElement(tSprID).setText(tStr)
          set the selStart to tStr.length
          set the selEnd to tStr.length
          return 1
        else
    ...
    case tSprID of
      "updatepw_cancel_button", "updatemail_cancel_button", "updateok_ok_button":
        removeWindow(tWndID)
      "updatepw_ok_button":
        tCurrPwd = pTempPasswords["char_currpwd_field"]
        tNewPwd = pTempPasswords["char_newpwd1_field"]
        ...
        tPw1 = []
        tPw2 = []
        if not voidp(pTempPasswords["char_newpwd1_field"]) then
          repeat with i = 1 to pTempPasswords["char_newpwd1_field"].length
            tPw1.append(chars(pTempPasswords["char_newpwd1_field"], i, i))
          end repeat
        end if
        if not voidp(pTempPasswords["char_newpwd2_field"]) then
          repeat with i = 1 to pTempPasswords["char_newpwd2_field"].length
            tPw2.append(chars(pTempPasswords["char_newpwd2_field"], i, i))
          end repeat
        end if
        if not me.checkPasswordsEnforced(tPw1, tPw2) then
          ...
          me.highlightVerifyTopic()
          pTempPasswords["char_newpwd1_field"] = EMPTY
          pTempPasswords["char_newpwd2_field"] = EMPTY
          return 0
        end if
        tWndObj.unmerge()
