...
on ChangeWindowView me, tWindowName 
  ...
  tWndObj.merge(tWindowName & ".window")
...
on updateRoomList me, tNodeId, tRoomList 
  ...
  tElement = tWndObj.getElement("nav_roomlist")
  if (tElement = 0) then
    return FALSE
  end if
  tElement.feedImage(tImage)
  tElement = tWndObj.getElement("nav_scrollbar")
  if (tElement = 0) then
    return FALSE
  end if
  tElement.setScrollOffset(0)
  return TRUE
end
...
on showNodeInfo me, tNodeId 
  ...
  if (tNodeInfo = 0) then
    ...
    if (tView = #unit) then
      ...
      tHeaderTxt = tNodeInfo.getAt(#name) && "(" & tNodeInfo.getAt(#usercount) & "/25)" & "\r"
      ...
      if (tView = "open") then
        ...
        if (tView = "closed") then
          ...
          if (tView = "password") then
            ...
            return(error(me, "Saved node data is not valid!", #showNodeInfo))
          end if
        end if
      end if
      if tWndObj.elementExists("nav_modify_button") then
...
on createCatItemImage tNum, tColor 
  tImg = image(307, 16, 8, member("nav_ui_palette"))
  tSrc = member("nav_rw_lf" & tNum).image
  tImg.copyPixels(tSrc, tSrc.rect, tSrc.rect)
  tImg.fill(6, 0, 307, 16, tColor)
  tSrc = member("nav_rw_lf" & tNum).image
  tImg.copyPixels(tSrc, [point(307, 0), point(301, 0), point(301, 16), point(307, 16)], tSrc.rect)
  tSrc = member("nav_rw_plus").image
  tImg.copyPixels(tSrc, rect(6, 4, 14, 12), tSrc.rect, [#ink:36])
  tSrc = member("nav_rw_arr").image
  tImg.copyPixels(tSrc, rect(282, 4, 289, 12), tSrc.rect, [#ink:36])
  tImg.copyPixels(tSrc, rect(289, 4, 296, 12), tSrc.rect, [#ink:36])
  tImg.copyPixels(tSrc, rect(296, 4, 303, 12), tSrc.rect, [#ink:36])
  return(tImg)
end
...
on createRoomItemImage tNum, tColor 
  ...
  tImg.copyPixels(tSrc, tSrc.rect, tSrc.rect)
  tImg.fill(6, 0, 242, 16, paletteIndex(82))
  tSrc = member("nav_rw_lf").image
  tImg.copyPixels(tSrc, [point(247, 0), point(241, 0), point(241, 16), point(247, 16)], tSrc.rect)
  tSrc = member("nav_rw_lf" & tNum).image
  tImg.copyPixels(tSrc, rect(249, 0, 255, 16), tSrc.rect)
  tImg.fill(255, 0, 301, 16, tColor)
  tSrc = member("nav_rw_lf" & tNum).image
  tImg.copyPixels(tSrc, [point(307, 0), point(301, 0), point(301, 16), point(307, 16)], tSrc.rect)
  tSrc = member("nav_rw_arr").image
  tImg.copyPixels(tSrc, rect(296, 4, 303, 12), tSrc.rect, [#ink:36])
