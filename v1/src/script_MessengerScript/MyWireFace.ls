on MyWireFace FigureData, FaceMember, small 
  ...
  repeat while faceParts <= 1
    f = getAt(1, count(faceParts))
    if voidp(small) or (small = 0) then
    ...
    faceColor = rgb(0, 0, 0)
    if faceParts <> "hd" then
      if faceParts <> "fc" then
        if (faceParts = "hr") then
...
on FigureDataParser FigureData 
  ...
  i = 1
  repeat while i <= count(PartList)
    model = getAt(PartList, i)
    the itemDelimiter = "/"
    setAt(PartList, i, model.item[1])
