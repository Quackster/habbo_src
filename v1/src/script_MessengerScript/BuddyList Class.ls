...
on handleFuseBuddyListMsg me, msg
  ...
  repeat while (i <= the number of lines in msg)
    ...
    if (ln.length > 4) then
      ...
      addProp(p, #unit, unit)
      addProp(p, #last_access_time, item 2 of msg)
...
on fillField me, num, p
  ...
  if (getaProp(p, #unit).length > 2) then
    ...
    location = ("Last visit" && getaProp(p, #last_access_time).char[1])
  end if
  ...
  set the textStyle of word 3 to the number of words in line 1 of field bField of field bField to "underline"
