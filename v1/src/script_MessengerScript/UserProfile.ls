...
on parseUserProfile data 
  ...
  mainlines = 0
  the itemDelimiter = "\t"
  num = the number of line in data
  i = 1
  repeat while i <= num
    id = integer(data.getPropRef(#line, i).getProp(#item, 1))
    parentid = integer(data.getPropRef(#line, i).getProp(#item, 2))
    name = data.getPropRef(#line, i).getProp(#item, 3)
    value = data.getPropRef(#line, i).getProp(#item, 4)
    if ((id mod 100) = 0) then
      if sub.length > 0 then
        gProfileP.addProp(oldMainId, ["text":oldMainName, "Checked":oldMainValue, "Visible":1, "Main":1, "Status":"Closed", "Multiroom":the number of line in sub])
