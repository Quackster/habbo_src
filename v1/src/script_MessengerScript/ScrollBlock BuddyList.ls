...
on exitFrame me 
  ...
  if scrollOn and the mouseDown then
    ...
    if scrollOn then
      scroll(gBuddyList, ((((nv - vOffset) - (sprite(gBLScrollBarSpr).locV - (sprite(gBLScrollBarSpr).height / 2))) * 1) / ((sprite(gBLScrollBarSpr).height - 28) - (vOffset * 2))))
    end if
    scrollOn = 0
  end if
end
...
on update me, f 
  vOffset = sprite(me.spriteNum).member.getProp(#regPoint, 2)
