...
on beginSprite me
  if (gPrivateDropStatus = VOID) then
    ...
    DropListItemA = ((me.spriteNum + value(sprite(me.spriteNum).member.name.char[1])) + 1)
...
on mouseDown me
  ...
  DropListV = (((value(sprite(me.spriteNum).member.name.char[1]) * 17) + (sprite(me.spriteNum).height / 2)) + 1)
  nextItemV = 0
  repeat with f = (me.spriteNum + 1) to (me.spriteNum + 5)
    sendSprite(f, #ActivateDropListItem, integer(((me.spriteNum + sprite(me.spriteNum).member.name.char[1]) + 1)))
    sendSprite(f, #SetMyLocVTo, (((sprite(me.spriteNum).locV - DropListV) + nextItemV) + value(sprite(me.spriteNum).member.name.char[1])))
