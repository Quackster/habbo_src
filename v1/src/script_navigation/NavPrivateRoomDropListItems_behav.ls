...
on SwapMember me
  if (sprite(me.spriteNum).member.name contains "_hi") then
    sprite(me.spriteNum).member = sprite(me.spriteNum).member.name.char[1]
  end if
end

on ActivateDropListItem me, NowAct
  if (me.spriteNum = NowAct) then
    if (sprite(me.spriteNum).member.name.char[(length(sprite(me.spriteNum).member.name) - 2)] <> "_hi") then
...
on mouseWithin me
  if (sprite(me.spriteNum).member.name.char[(length(sprite(me.spriteNum).member.name) - 2)] <> "_hi") then
    sendSprite(((gDropListSpr + value(sprite(gDropListSpr).member).name.char[1]) + 1), #SwapMember)
    sprite(me.spriteNum).member = (sprite(me.spriteNum).member.name & "_hi")
    sprite(gDropListSpr).member = (((me.spriteNum - gDropListSpr) - 1) & sprite(gDropListSpr).member.name.char[2])
  end if
end
...
on mouseUp me
  ...
  if (sprite(me.spriteNum).member.name.char[(length(sprite(me.spriteNum).member.name) - 2)] = "_hi") then
    put (((me.spriteNum - gDropListSpr) - 1) & sprite(gDropListSpr).member.name.char[2])
    gPrivateDropMem = the number of member (((the spriteNum of me - gDropListSpr) - 1) & the name of the member of sprite(gDropListSpr).char[2])
