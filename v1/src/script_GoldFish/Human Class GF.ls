...
on new me, tName, tMemberPrefix, tMemberModels, tLocX, tLocY, tHeight, tdir, tlDimensions, tSpr, tCustom
  ...
  iLocZFix = 0
  sleeping = 0
  if (the movieName.char[1] <> "gf_private") then
    peopleSize = "sh"
  else
    peopleSize = "h"
  end if
  ...
  repeat with i = 1 to count(pModels)
    ...
    if ((model = "998") and (tName <> "Kolumbus")) then
      model = "001"
    end if
    if ((model = "987") and (tName <> "DjEsko")) then
      model = "001"
    end if
  ...
  if ((counter mod 2) = 0) then
    ...
    if moving then
      ...
      if (((dir = 0) or (dir = 1)) or (dir = 2)) then
        sprite(pSprites.lh).locZ = (sprite(pSprites.ch).locZ - 10)
        sprite(pSprites.ls).locZ = (sprite(pSprites.ch).locZ - 10)
      else
        if (((dir = 4) or (dir = 5)) or (dir = 6)) then
          sprite(pSprites.rh).locZ = (sprite(pSprites.ch).locZ - 10)
