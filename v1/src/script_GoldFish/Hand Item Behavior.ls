property shadowSpr, myUserObj, itemType, stripId, type, helpText, partColors
...
on setItem me, ttype, tid, titemType, tpartColors 
  ...
  repeat while t <= the number of item in tpartColors
    add(tlpartColors, string(tpartColors.item[t]))
    t = (1 + t)
  end repeat
  ...
  if (ttype = "post.it") and gpPostItNos <> void() then
    if (getProp(gpPostItNos, stripId) = void()) then
      ...
      Postnums = integer((getProp(gpPostItNos, stripId) / (20 / 6)))
    end if
    if Postnums > 6 then
      Postnums = 6
    end if
  ...
  if getmemnum(ttype & "_small") > 0 then
    ...
    if offset("*", ttype) > 0 then
      ttype = ttype.char[1..(offset("*", ttype) - 1)]
      sprite(me.spriteNum).castNum = getmemnum(ttype & "_small")
      put(ttype & "_small")
    end if
  end if
  if string(tlpartColors.getAt(count(tlpartColors))) starts "*" then
    sprite(me.spriteNum).bgColor = rgb("#" & string(tlpartColors.getAt(count(tlpartColors))).char[2..string(tlpartColors.getAt(count(tlpartColors))).length])
  else
    sprite(me.spriteNum).bgColor = paletteIndex(integer(tlpartColors.getAt(count(tlpartColors))))
  end if
  ...
  if type contains "post.it" and gpPostItNos <> void() then
    ...
    if getmemnum(type & "_small") > 0 then
      ...
      if offset("*", type) > 0 then
        type = type.char[1..(offset("*", type) - 1)]
        sprite(me.spriteNum).castNum = getmemnum(type & "_small")
        put(type & "_small")
      end if
    end if
