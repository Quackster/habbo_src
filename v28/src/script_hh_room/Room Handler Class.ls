  ...
  repeat while me <= undefined
    ...
    if not pRemoteControlledUsers.getOne(tuser.getAt(#id)) > 0 then
      ...
      if tUserObj <> 0 then
        ...
        tAllowFX = call(#validateFxForActionList, [tUserObj], tActionIndex)
        if tAllowFX <> 1 then
...
on handle_items(me, tMsg)
  ...
  repeat while i <= tMsg.count(#line)
    ...
    if tLine <> "" then
      ...
      tClassID = value(tLine.getProp(#item, 2))
  ...
  repeat while i <= tCount
    ...
    if tItem = "" then
      ...
      if tItem.count(#item) < 2 then
        ...
        tClassID = value(tItem.getProp(#item, 6))
        if me = "S" then
          ...
          if tItem.getProp(#item, 10) <> "" and tItem.count(#item) >= 10 then
            tObj.setAt(#slotID, tItem.getProp(#item, 10))
          end if
          if tItem.getProp(#item, 11) <> "" and tItem.count(#item) >= 11 then
            tObj.setAt(#songID, tItem.getProp(#item, 11))
          end if
          the itemDelimiter = ","
