...
on define me, tdata 
  ...
  pLocY = tdata.getAt(#y)
  pLocH = pAltitude
  pExpireTimeStamp = tdata.getAt(#expire)
...
on getInfo me 
  ...
  if not voidp(tFurniData) then
    tInfo.setAt(#name, pPersistentFurniData.getPropsByClass("s", pClass).getAt(#localizedName))
    tInfo.setAt(#custom, pPersistentFurniData.getPropsByClass("s", pClass).getAt(#localizedDesc))
  else
    if pClass contains "placeholder" then
      tInfo.setAt(#name, getText("furni_active_placeholder_name"))
      tInfo.setAt(#custom, getText("furni_active_placeholder_desc"))
    else
      tInfo.setAt(#name, "")
      tInfo.setAt(#custom, "")
    end if
  end if
  tInfo.setAt(#expire, pExpireTimeStamp)
...
on rotate me, tChange 
  tName = sprite(pSprList.getAt(1)).member.name
  ...
  repeat while j <= 3
    ...
    if tNameExploded.count < 2 then
      ...
      tNameExploded.setAt((tNameExploded.count - 1), string(tDirection.getAt(1)))
...
on capturesEvents me, tPart, tClass 
  ...
  if ilk(tPropList) <> #propList then
    ...
    if not voidp(tPropList.getAt(tPart).getAt(#events)) then
      return(tPropList.getAt(tPart).getAt(#events))
    end if
  end if
  return TRUE
end
