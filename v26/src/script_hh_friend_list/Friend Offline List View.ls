...
on renderListImage(me)
  ...
  repeat while me <= undefined
    ...
    if me.isFriendselected(tName) then
      me.fill(0, tCurrentPosV, pItemWidth, tCurrentPosV + pItemHeight, tSelectedBg)
    end if
...
on renderFromQueue(me, tContentElement)
  ...
  repeat while i <= me.pTasksPerUpdate
    if me.count(#pFriendRenderQueue) > 0 then
      ...
      tName = tFriend.getAt(#name)
      if me.isFriendselected(tName) then
        me.fill(0, tCurrentPosV, pItemWidth, tCurrentPosV + pItemHeight, rgb(string(getVariable("fr.offline.bg.selected"))))
      end if
