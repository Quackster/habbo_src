property pGeometryId, pContainerID, pSafeTraderID, pArrowObjID, pObjMoverID, pPreviewObjID, pDoorBellID, pRoomGuiID, pIgnoreListID, pRespectMgrID, pLoaderBarID, pRoomSpaceId, pWideScreenOffset, pHiliterId, pSwapAnimations, pInterfaceId, pQueueCollection, pLoadingBarID, pWIndowText, pCoverSpr, pSelectedObj, pClickAction, pSongSelectorID, pJudgeToolID, pModBadgeList, pSelectedType, pDelConfirmID, pPlcConfirmID, pDeleteType, pDeleteObjID, pBannerLink, pInfoConnID
...
on resizeInterstitialWindow me 
  ...
  repeat while tElementList <= undefined
...
on getProperty me, tPropID 
  if (tPropID = #clickAction) then
    ...
    if (tPropID = #widescreenoffset) then
...
on eventProcRoom me, tEvent, tSprID, tParam 
  ...
  if (tEvent = #mouseUp) and tSprID contains "command:" then
    ...
    tPrm = [:]
    if (tCmd = "MOVE") then
      tPrm = [#integer:integer(tSprID.getProp(#word, 3)), #integer:integer(tSprID.getProp(#word, 4))]
    else
      if (tCmd = "GOAWAY") then
        tPrm = [:]
      else
  ...
  if (tEvent = #mouseDown) or tDragging then
    if (tCmd = "moveHuman") then
      ...
      if listp(tloc) then
        return(me.getComponent().getRoomConnection().send("MOVE", [#integer:tloc.getAt(1), #integer:tloc.getAt(2)]))
      end if
    else
...
on eventProcUserObj me, tEvent, tSprID, tParam 
  ...
  if tObject.select() then
    ...
    if tObject <> me.getComponent().getOwnUser() or tObject.getProperty(#moving) then
      me.getComponent().getRoomConnection().send("LOOKTO", [#integer:integer(tloc.getAt(1)), #integer:integer(tloc.getAt(2))])
    end if
  else
    pSelectedObj = ""
    pSelectedType = ""
