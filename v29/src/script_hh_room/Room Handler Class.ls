...
on handle_error(me, tMsg)
  ...
  if me = "info: No place for stuff" then
    ...
    if length(tLine) > 5 then
      ...
      repeat while j <= tLine.count(#item)
        if length(tLine.getProp(#item, j)) > 1 then
          tActionName = tLine.getPropRef(#item, j).getProp(#word, 1)
          tActions.add([#name:tActionName, #params:tLine.getProp(#item, j)])
              ...
              if me = "s" then
                ...
                if me = "p" then
                  ...
                  if me = "b" then
                    ...
                    tPairNum = 1
                    repeat while tPairNum <= tDataPairs.count
                      tPair = explode(tDataPairs.getAt(tPairNum), ":")
                      if tPair.count < 2 then
                      else
                        tBadges.setaProp(integer(tPair.getAt(1)), tPair.getAt(2))
                      end if
                      tPairNum = 1 + tPairNum
                    end repeat
                    tList.getAt(tuser).setAt(#badge, tBadges)
                  else
                    if me = "a" then
                      ...
                      if me = "g" then
                        ...
                        if me = "t" then
                          ...
                          if me = "x" then
                            tList.getAt(tuser).setAt(#xp, integer(tdata))
                          else
                            if me = "d" then
                              tdir = integer(tdata)
                              tList.getAt(tuser).setAt(#direction, [tdir, tdir])
                            else
...
on handle_items(me, tMsg)
  ...
  repeat while i <= tMsg.count(#line)
    ...
    if tLine <> "" then
      ...
      tClassID = integer(tLine.getProp(#item, 2))
  ...
  repeat while i <= tCount
    ...
    if tItem = "" then
      ...
      if tItem.count(#item) < 2 then
        ...
        tClassID = integer(tItem.getProp(#item, 6))
        if not integerp(tClassID) then
          tClassID = tItem.getProp(#item, 6)
        end if
        if me = "S" then
          ...
          tObj.setAt(#stuffdata, tItem.getProp(#item, 7))
          tObj.setAt(#isRecyclable, tItem.getProp(#item, 8))
          tObj.setAt(#isTradeable, tItem.getProp(#item, 9))
          if tItem.count(#item) >= 10 then
            tObj.setAt(#expire, tItem.getProp(#item, 10))
          end if
          if tItem.getProp(#item, 11) <> "" and tItem.count(#item) >= 11 then
            tObj.setAt(#slotID, tItem.getProp(#item, 11))
          end if
          if tItem.getProp(#item, 12) <> "" and tItem.count(#item) >= 12 then
            tObj.setAt(#songID, tItem.getProp(#item, 12))
          end if
          the itemDelimiter = ","
...
on handle_trade_items(me, tMsg)
  ...
  tMessage = []
  i = 1
  repeat while i <= tMsgParts.count
    tLine = tMsgParts.getAt(i)
    tdata = []
    tDelim = the itemDelimiter
    the itemDelimiter = ";"
    tdata.setAt(#accept, tLine.getProp(#item, 2))
    tItemStr = "foo" & "\r" & tLine.getProp(#item, 3, tLine.count(#item)) & "\r" & 1
    ...
    tUserName = tLine.getProp(#item, 1)
    if tUserName = "" then
...
on handle_trade_accept(me, tMsg)
  ...
  tStatus = tConn.GetIntFrom() = 1
  me.getInterface().getSafeTrader().accept(tUserName, tStatus)
  exit
end
