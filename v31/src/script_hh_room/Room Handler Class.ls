...
on handle_old_stripinfo me, tMsg, tItemDeLim 
  ...
  tDelim = the itemDelimiter
  tMsg.setAt(content, integer(#line.getProp(tMsg, content.count(#line))))
  the itemDelimiter = "/"
  tCount = content.count(#item)
  tStripMax = 0
  tTotalItemCount = 0
  i = 1
  repeat while i <= tCount
    the itemDelimiter = "/"
    tItem = content.getProp(#item, i)
    if tItem = "" then
    else
