...
on renderSmallIcons me, tstate, tPram 
  ...
  if tstate <> #hilite then
    ...
    tPageItemDownloader = getThread(#catalogue).getComponent().getPageItemDownloader()
    f = tFirst
      ...
      if not voidp(pPageData.getAt(f).getAt(1).getAt("smallPrewImg")) then
        ...
        if tmember <> 0 then
          if tWndObj.elementExists(tID) then
            ...
            tClass = pPageData.getPropAt(f)
            if not tPageItemDownloader.isAssetDownloading(tClass) then
              tTempSmallImg = getObject("Preview_renderer").renderPreviewImage(void(), void(), "", tClass)
            else
              tTempSmallImg = pPageData.getAt(f).getAt(1).getAt("smallPrewImg")
            end if
