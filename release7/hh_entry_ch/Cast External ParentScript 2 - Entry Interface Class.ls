...
on showHotel me
  if not visualizerExists(pEntryVisual) then
    ...
    tSprA = tVisObj.getSprById("elevator1")
    tSprB = tVisObj.getSprById("elevator2")
    if (tSprA <> 0) and (tSprB <> 0) then
      tObj = createObject(#temp, "Entry Elevator Class")
      tObj.define(tSprA, tSprB)
      pItemObjList.add(tObj)
    end if
