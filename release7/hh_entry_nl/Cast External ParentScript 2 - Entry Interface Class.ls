property pEntryVisual, pBottomBar, pSignSprList, pSignSprLocV, pItemObjList, pUpdateTasks, pViewMaxTime, pViewOpenTime, pViewCloseTime, pAnimUpdate, pFirstInit, pInActiveIconBlend, pMessengerFlash, pNewMsgCount, pNewBuddyRequests, pClubDaysCount, pWaterAnimConter, pFrameCounter
...
on showHotel me
  if not visualizerExists(pEntryVisual) then
    ...
    repeat with i = 1 to 5
      tSpr = tVisObj.getSprById("mill_" & i)
      if tSpr <> 0 then
        tObj = createObject(#temp, "Entry Mill Class")
        tObj.define(tSpr, i)
        pItemObjList.add(tObj)
      end if
    end repeat
    tSpr = tVisObj.getSprById("train")
    if tSpr <> 0 then
      if not objectExists("TrainMover") then
        tObj = createObject("TrainMover", "Entry Train Class")
        tObj.define(tSpr)
      end if
    end if
...
on hideHotel me
  ...
  pItemObjList = []
  if objectExists("TrainMover") then
    removeObject("TrainMover")
  end if
...
on prepare me
  ...
  if pAnimUpdate then
    ...
    tSpr = tVisual.getSprById("water")
    tMem = tSpr.member
    tMem.paletteRef = member(getmemnum("water" & tFrame & "_palette"))
    pFrameCounter = 0
