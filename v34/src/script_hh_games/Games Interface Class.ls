...
on setupChessPieces me, tGameProps
  ...
  repeat with i = 1 to tGameProps[#gameboard].word.count
    if (tGameProps[#gameboard].word[i].length = 5) then
      tC = ("game_" & tGameProps[#gameboard].word[i].char[1])
      tPieceCoordinate = tGameProps[#gameboard].word[i].char[4]
...
on makeMoveChess me, tpoint, tPieceData
  ...
  tMove = ((tPieceData.char[4] && numToChar((97 + value(tClickArea.char[1])))) & (8 - value(tClickArea.char[2])))
  me.getComponent().makeMoveChess(tMove)
  tC = ("game_" & tPieceData.char[1])
  ...
  if (tEvent = #mouseUp) then
    case tElemID of
      ...
      "game_area":
        ...
        tMemName = ("game_" & pChessPieces[tClickArea].char[1])
