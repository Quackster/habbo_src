...
global gBuddyList, gMessageManager, gpUiButtons, gPopUpContext, gChosenBuddyId, gChosenbuddyName, gpBuddyIndicators, gpBuddyExistsIndicators

on beginSprite me
  put EMPTY into field "receivers"
  member("receivers.show").text = AddTextToField("receivers2")
...
on mouseUp me
  ...
  if (the mouseV > (sprite(me.spriteNum).top + 15)) or (getBuddyMsgCount(gMessageManager, getBuddyIdFromFieldNum(gBuddyList, num)) = 0) then
    ...
    if the doubleClick then
      ...
      goContext("writemsg", gPopUpContext)
    end if
  else
    ...
    goContext("readmsg", gPopUpContext)
    msg = getNextBuddyMsg(gMessageManager, gChosenBuddyId)
    if msg = VOID then
      goContext("buddies", gPopUpContext)
      return 
    end if
    display(msg)
  end if
...
on disable me
  ...
  if field("receivers").length < 3 then
