property ancestor, fireplaceOn, polyfonfprand, formulaFrame, formulaMode, tvFrame, carLoop, carLoopCount, stillPicture, stillWait
...
on exitFrame me
  ...
  if fireplaceOn and ((tvFrame mod 2) = 1) then
    mname = me.lSprites[4].member.name
    the itemDelimiter = "_"
    tmpName = EMPTY
    ...
    if getmemnum(newMName) > 0 then
      me.lSprites[4].castNum = getmemnum(newMName)
    end if
  end if
  ...
  me.lSprites[4].locZ = me.lSprites[1].locZ + 2
end
