...
global xoffset, yoffset, xSize, ySize, hiliter, lasth, lastV, gUserSprites, gChosenUser, gChosenUserSprite, gChosenObject, gUserColors, gXtraFeatureSprite, gChosenStripLevel, gXFactor, gYFactor, gHFactor, gpUiButtons, gGameContext, gMyName, gpObjects
...
on beginSprite me
  ...
  set the member of sprite hiliteSprite to "memberhilite_small_0"
...
on mouseDown me, brokenClickFromItem
  ...
  if voidp(o) then
    ...
    if sprite(me.spriteNum).locH > -1 then
...
on hiliteAvatar me, ChosenObject, whichSprite
  if (ChosenObject.name <> "Piers") and (ChosenObject.name <> "Maria") and (ChosenObject.name <> "James") and (ChosenObject.name <> "Dave") and (ChosenObject.name <> "Sadie") and (ChosenObject.name <> "Marion") then
    repeat with part in ChosenObject.lParts
      ...
      if (the movieName).char[1..2] <> "cr" then
        sprite(whichSprite).loc = point(685, 392)
      else
        sprite(whichSprite).loc = point(695, 345)
      end if
