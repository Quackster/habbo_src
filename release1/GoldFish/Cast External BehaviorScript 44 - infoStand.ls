      ...
      case placingType of
        #stuff:
          ...
          if objectp(giveStuffTo) then
            fusePString = "GIVESTRIPITEM" && placingStuffStripId && giveStuffTo.name
          end if
        #item:
        ...
        #stuffMove:
          ...
          if objectp(giveStuffTo) then
            put "GIVESTUFFTO"
            fusePString = "GIVESTUFFTO" && placingObject.id && giveStuffTo.name
          end if
      end case
...
on setUserInfoTexts me
  ...
  member("item.info_text").text = getCustom(gChosenObject)
