property pEntryVisual, pSignSprList, pItemObjList, pBottomBar, pAnimUpdate, pFrameCounter, pWaterAnimConter, pUpdateTasks, pFirstInit, pViewMaxTime, pViewOpenTime, pViewCloseTime, pNewMsgCount, pNewBuddyRequests, pMessengerFlash, pInActiveIconBlend
...
on showHotel me 
  if not visualizerExists(pEntryVisual) then
    ...
    i = 1
    repeat while 1
      tSpr = tVisObj.getSprById("small_fountain" & i)
      if tSpr <> 0 then
        tObj = createObject(#temp, "Entry Fountain Class")
        tObj.define(tSpr, "Fountain_it_")
        pItemObjList.add(tObj)
      else
      end if
      i = i + 1
...
on prepare me 
  ...
  if pAnimUpdate then
    ...
    if pFrameCounter > 3 then
      ...
      tMem = tSpr.member
      tMem.paletteRef = member(getmemnum("dk_water_palette_" & tFrame))
      pFrameCounter = 0
    end if
