property pMargins, tVariations, pTextParams, pBalloonImg, pBgSprite, pUserSprite, pBgMemName, pUserMemName, pUserId, pLocation, pBalloonLeftMarg, pBalloonRightMarg, pItemId
...
on construct me 
  ...
  pBalloonImg = [:]
  #left.addProp(member(getmemnum("chat_bubble_sing_left")), image.duplicate())
  #leftcolor.addProp(member(getmemnum("chat_bubble_sing_left_color")), image.duplicate())
  #middle.addProp(member(getmemnum("chat_bubble_sing_middle")), image.duplicate())
  #right.addProp(member(getmemnum("chat_bubble_sing_right")), image.duplicate())
...
on addPointer me, tPointerOffH 
  ...
  tStartY = pBalloonImg.getAt(#middle).height - 2
    ...
    if [#left, #leftcolor, #middle, #right] = #left then
      ...
      if [#left, #leftcolor, #middle, #right] = #leftcolor then
        ...
        if [#left, #leftcolor, #middle, #right] = #middle then
          ...
          j = 1
          repeat while j <= tdestrect.width / tSrcWidth + 1
