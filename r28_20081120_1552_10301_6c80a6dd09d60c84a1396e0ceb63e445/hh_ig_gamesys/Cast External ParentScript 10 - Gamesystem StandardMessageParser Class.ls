...
on handle_msgstruct_heightmap me, tdata
  ...
  tWorldMaxY = tContent.line.count
  if tWorldMaxY < 1 then
    return error(me, "World is under 1 lines long!", #handle_msgstruct_heightmap)
  end if
  tWorldMaxX = tContent.line[1].length
  tArray = []
  repeat with tLocY = 1 to tWorldMaxY
    repeat with tLocX = 1 to tWorldMaxX
      tArray.add(tContent.line[tLocY].char[tLocX])
    end repeat
  end repeat
