...
on Init me, tSourceURL, tClickURL, tRegisteredLayout 
  if tSourceURL <> 0 then
    ...
    tSourceURL = tSourceURL & tSeparator & "r=" & random(9999999999)
    ...
    if tVisObj.spriteExists("billboard_bg") then
      ...
      tSprBg.paletteRef = member(getmemnum("adframe_palette1"))
      if tSprBg.name contains "left" then
...
on adReady me 
  ...
  if tVisObj.spriteExists("billboard_img") then
    ...
    if tVisObj.spriteExists("billboard_bg") then
      ...
      tSpr.paletteRef = member(getmemnum("adframe_palette2"))
    end if
